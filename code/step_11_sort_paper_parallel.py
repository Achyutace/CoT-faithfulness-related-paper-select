...
- Large Language Models Don't Always Say What They Think (Turpin et al.)
...
def analyze_all_papers_parallel(papers, selected_papertitles, client, max_workers = 20):
    '''
    analyze_all_papers_parallel 的 Docstring
    
    :param papers: papers的列表，每个paper为json格式
    :param selected_papertitles: 需要过得papertitles列表
    ...
    results_data = []
    tasks = [p for p in papers if p['title'] in selected_papertitles]
    print(f"需要分析的论文数量: {len(tasks)}, 并发数: {max_workers}, 开始分析...")

    def processing_single_paper(paper):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_paper = {executor.submit(processing_single_paper, paper):paper for paper in tasks}
        for future in tqdm(concurrent.futures.as_completed(future_to_paper), total = len(tasks), desc="Processing Papers"):
            ...
            try:
                ...
                print(f"Paper {paper['title']} generated an exception: {exc}")
            ...
            for cite in citations:
                if cite.paperId not in seen_ids:
                    ...
                    if cite.title and cite.abstract:
                        venue_info = ""
                        if cite.publicationVenue:
                            # 尝试获取 venue 的名字，如果对象有 'name' 属性则取名，否则转为字符串
                            venue_info = getattr(cite.publicationVenue, 'name', str(cite.publicationVenue))
...
def parallel_main():
    # 1. 获取数据
    papers = fetch_citations_with_cache(SEED_PAPERS, cache_file="citation_cache.json")
    last_select_f = pd.read_csv("data/citations_select/cot_faithfulness_citations_select_11.csv")

    # 调试：取消过滤，全部跑一遍
    true_titles = last_select_f['title'].tolist()
    # 创造true列表，包含所有is_relevant为true的title
    # true_titles = last_select_f[last_select_f['is_relevant'] == True]['title'].tolist()
    # 调试模式：只跑前 5 篇测试
    # papers = papers[:5] 
    ...
    results_data = []
    filename = "cot_faithfulness_citations_select_15.csv"

    print("开始调用 DeepSeek V3 进行并行分析...")

    results_data = analyze_all_papers_parallel(papers, true_titles, client, max_workers=20)
